...
class NamesMixin:
    ...
    def names(self, request, *args, **kwargs):
        if self.basename == 'projects':
            queryset66 = Projects.objects.all()
        else:
            queryset66 = Envs.objects.all()

        queryset77 = self.filter_queryset(queryset66)
        page = self.paginate_queryset(queryset77)
        if page is not None:
            serializer = self.get_serializer(page, many=True)
            return self.get_paginated_response(serializer.data)

        serializer = self.get_serializer(queryset77, many=True)
